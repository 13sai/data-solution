...
class Medoo
{
    ...
    protected function dataImplode(array $data, array &$map, string $conjunctor): string
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if (
                $type === 'array' &&
                preg_match("/^(AND|OR)(\s+#.*)?$/", $key, $relationshipMatch)
            ) {
                $stack[] = '(' . $this->dataImplode($value, $map, ' ' . $relationshipMatch[1]) . ')';
                continue;
            }
    ...
    protected function buildJoin(string $table, array $join, array &$map): string
    {
        ...
        foreach ($join as $subtable => $relationship) {
            ...
            if (is_string($relationship)) {
                $relationship = 'USING ("' . $relationship . '")';
            } elseif (is_array($relationship)) {
                // For ['column1', 'column2']
                if (isset($relationship[0])) {
                    $relationship = 'USING ("' . implode('", "', $relationship) . '")';
                } else {
                    $joins = [];
                    ...
                    $relationship = 'ON ' . implode(' AND ', $joins);
                }
            } elseif ($raw = $this->buildRaw($relationship, $map)) {
                $relationship = $raw;
            }
            ...
            $tableJoin[] = $type[$match['join']] . " JOIN ${tableName} ${relationship}";
        }
